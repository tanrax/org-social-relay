...
class Profile(models.Model):
    ...
    description = models.TextField(blank=True, help_text="Short description about the profile")
    avatar = models.URLField(blank=True, help_text="URL to avatar image (128x128px JPG/PNG)")
...
class ProfileContact(models.Model):
    ...
    contact_type = models.CharField(max_length=50, help_text="Type of contact (email, xmpp, matrix, etc.)")
...
class Follow(models.Model):
    ...
    nickname = models.CharField(max_length=100, blank=True, help_text="Optional nickname for the followed profile")
    created_at = models.DateTimeField(auto_now_add=True)
...
class Post(models.Model):
    ...
    post_id = models.CharField(max_length=50, help_text="Unique timestamp identifier (RFC 3339 format)")
    content = models.TextField(help_text="Post content")
    
    # Optional properties
    language = models.CharField(max_length=10, blank=True, help_text="Language code (LANG property)")
    tags = models.CharField(max_length=500, blank=True, help_text="Space-separated tags")
    client = models.CharField(max_length=100, blank=True, help_text="Client application used")
    reply_to = models.CharField(max_length=300, blank=True, help_text="ID of post being replied to (URL#ID format)")
    mood = models.CharField(max_length=10, blank=True, help_text="Mood indicator (emoji)")
    
    # Poll related fields
    poll_end = models.DateTimeField(blank=True, null=True, help_text="End time for polls")
...
class PollVote(models.Model):
    ...
    poll_post = models.ForeignKey(Post, on_delete=models.CASCADE, related_name='votes_received', help_text="The original poll post")
